...
from finder_service.code.parsers.music_parser import AbstractParser, TrackMatch, DecodeError, HTMLStructureError
...
class PartyParser(AbstractParser):
    ...
    _permission_url = "https://mp3party.net/song_permissions/"
    ...
    async def _get_variants(self, artist: str, track: str, duration: int) -> list[dict]:
        ...
        for text in results:
            ...
            for i in a:
                try:
                    ...
                    id = url[url.rfind('/')+1:url.rfind('.mp3')]

                    ids_for_permission_request.append(id)
        ...
        if len(ids_for_permission_request) > 0:
            await self._session.post(self._permission_url, json={"ids": ids_for_permission_request})
